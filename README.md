...
结果 IP 的前两段来自 `gateway`。例如 gateway 为 `10.10.0.1` 时，映射值 4 会生成 `10.10.4.x`。
...
- 挂载持久化统一写入 `/etc/fstab`，并自动追加 `_netdev,nofail`；写入后会执行 `systemctl daemon-reload` 和 `mount -a -t nfs`；不再生成 systemd `.mount/.automount`。
